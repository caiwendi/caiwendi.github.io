...
11. 整个语义分割的后期处理过程采用高斯条件随机场，我们将条件随机场的能量函数定义为下面形式，要保证全局最小的条件是![](http://ww1.sinaimg.cn/large/b4c0024fgy1fkjvkl473yg203000i0d9.gif), 为了达到此条件让整个模型收敛，可以通过链式求导规则求解出A和B的梯度。
12. 下面我们采用了另一种方法可以让整个模型收敛的更快, 通过Potts-type pairwise模型将二元能量形式转化为每个像素匹配一个标签，再将收敛条件的等式转化为矩阵形式，
13. 然后通过链式求导法和一系列推导得出![](http://ww1.sinaimg.cn/large/b4c0024fgy1fkjvlkb28vg200d00h04b.gif)的梯度。
14. 以上得到权重的梯度后，最后用Softmax进行分类，对每个像素和标签的匹配给出概率值，这些概率值由交叉熵进行惩罚处理，最后随着训练次数的增加，交叉熵的损失值越小，模型的预测会越精准。
15. 下面对实验的部分进行介绍，数据集采用PASCAL VOC 2012,分为训练集，验证集和测试集，一共有20个物体类和1个背景，分割的标准是交除并，在训练过程中，我们采用在ImageNet上预训练好的DCNN，交叉熵损失值的下降方式采用共轭梯度下降，经过不断试验最后选定的Mini-batch = 20, Momentum = 0.9, Weight decay =![](http://ww1.sinaimg.cn/large/b4c0024fgy1fkjvly8flpg201h00g088.gif) , Learning rate = 0.001并且每迭代2000次学习率降低10倍。
...
11. In the post-processing of the whole semantic segmentation, we use the Gaussian conditional random field. We define the energy function of the conditional random field as the following form. To ensure that the global minimum condition is ![](http://ww1.sinaimg.cn/large/b4c0024fgy1fkjvkl473yg203000i0d9.gif) and the whole model convergence,  the gradient of A and B can be solved in terms of chaining rule.
12. Here we use another method to make the whole model converge faster. A Potts-type pairwise model is
    proposed to describe by the  following equation , the pairwise energy term is denoted for ![](http://ww1.sinaimg.cn/large/b4c0024fgy1fkjvts3zpeg200e00c048.gif) pixel taking the ![](http://ww1.sinaimg.cn/large/b4c0024fgy1fkjvu9oq6lg200900g03l.gif)label, and  ![](http://ww1.sinaimg.cn/large/b4c0024fgy1fkjvujhwytg200g00e04m.gif) pixel taking the ![](http://ww1.sinaimg.cn/large/b4c0024fgy1fkjvutl8pag200b00j03y.gif) label.To derive the inference and gradient equations, the inference equation ![](http://ww1.sinaimg.cn/large/b4c0024fgy1fkjvkl473yg203000i0d9.gif) is rewritten as Matrix form.
13. And then through the chain derivation rule and a series of inferences, we can get  ![](http://ww1.sinaimg.cn/large/b4c0024fgy1fkjvlkb28vg200d00h04b.gif)  gradient.
